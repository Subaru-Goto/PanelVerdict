...
| default executor on the deployed 1-vCPU container | 5 workers | `min(32, 1 + 4)`; measured on #242, 2026-08-30 |
...
**The executor was the smaller ceiling, by accident.** Five workers fronted fifteen
connections. With every new connection's `getaddrinfo` in the same pool, a saturated
...
1. **The shared executor is sized to the pool it fronts.** At startup the loop's
   default executor gets `pooler_pool_size` workers (15 here). It fronts that many
   connections and must never be the smaller ceiling by accident; threads waiting on
   the network are cheap. The number is a setting with the dashboard reading as its
...
2. **Every connection opens under one deadline, 3 s.** `/health` and the seed already
   used it; `get_conn` now does too, from one constant in `db.py`. Evidence it is
   enough: the keep-warm ping opens a connection under it 120 times a day and must
   answer `"db":"up"`. Evidence it is right: at a full pool the pooler would otherwise
...
3. **Saturation is a status, not a traceback.** A request whose connection cannot
   ...
   a moment." The driver's words stay out of it. Raised where the connection opens,
   so a connection lost mid-query keeps its own error.
