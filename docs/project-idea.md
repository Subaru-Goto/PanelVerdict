...
- **Chance is the wrong denominator — the ceiling the labels allow is (added 2026-08-26; [102](https://github.com/Subaru-Goto/PanelVerdict/issues/218)).** An Upworthy pair's "true winner" is itself an estimate from finite impressions, so on every pair whose CTR gap sits inside sampling noise *no* predictor can be reliably right: 50% is the bar for a coin, on labels that are partly coins themselves. The ceiling is computable from the archive alone — before any panel runs, at zero API cost. Per pair, draw each variant's rate from its Beta posterior, simulate a *replication* of the same test at the same impression counts, and record how often the replication reproduces the observed winner; the mean over the benchmark sample is the **maximum expected accuracy of any predictor against these labels**. "62% against a 74% achievable ceiling" is both honester and stronger than "62% vs. chance"; it converts dropping or down-weighting coin-flip pairs from a post-hoc excuse into a pre-registered rule; and it sets the MDE against the attainable range instead of against 100%. Report its limits alongside it: this is label reliability at Upworthy's sample sizes, not a bound on the underlying preference; and filtering by reliability shifts the test population toward large-effect pairs, so filtered and unfiltered accuracy are two different numbers and both get published. Distinct from the human test–retest ceiling the synthetic-audience vendors quote (Artificial Societies publishes 86% against a 91% human self-replication level) — the same corrective, a different construct: theirs is people disagreeing with themselves, ours is sampling noise in a click experiment.
...
**Current chapter (2026-08-21):** the map is [078 · next chapter (#122)](https://github.com/Subaru-Goto/PanelVerdict/issues/122) — PanelVerdict deployed to production on a public URL, satisfying the revised requirement set: **an agent, LangGraph, RAG, human-in-the-loop, deployed, production-ready**. It redraws and closes the public-demo map ([055](decisions/055-map-public-demo.md)), lifting that map's "demo, not real production" ceiling; the human-in-the-loop requirement lands as the panel preview — accept or adjust before any vote is bought (`redraw` was cut in 077: filtering happens before sampling, so it could never change who matched) ([076](https://github.com/Subaru-Goto/PanelVerdict/issues/166), [077](https://github.com/Subaru-Goto/PanelVerdict/issues/167)). The phases below are the earlier record.
...
- **Evaluation:** three distinct layers, kept separate:
  1. *Pipeline QA (DeepEval):* structured-output validity, reason relevance/faithfulness, position-bias regression tests (same pair, swapped order → same verdict), persona-consistency checks. *(Split 2026-09-05, [110/#238](https://github.com/Subaru-Goto/PanelVerdict/issues/238). **Every push, free:** the DeepEval cases in `backend/evals/`, run under an overlay (`evals/requirements.txt`) because DeepEval's caps would downgrade the production image if it joined the lock — structured-output validity over the 300 recorded demo votes, and the first-position rate once a capture records the order (today a strict xfail, so the first capture that does fails loudly until a band is set). No client exists in that directory, so nothing there can spend. **On demand, paid, price stated first:** Ragas over 30 hand-written questions, measured $0.0993 a run (`docs/research/rag-baseline.md`); 091's topic-boundary harness, ~$0.015 per 48 cases; the seed's plausibility G-Eval, which stays the thin custom judge 006d/006e specified — the metric is unchanged, only new work uses DeepEval. The DeepEval suite carries no LLM-judged metric. **Deferred, not dropped:** the prompt-injection red team with attack-success rate tracked over time — 100/#209's threat model names the assets; the harness for it is unbuilt and owed its own ticket.)*
