...
| **analyst, inside** | `ModelCallLimitMiddleware(thread_limit=40, run_limit=8, exit_behavior='end')` | `run_limit=8` was what `2 * len(tools) + 2` evaluated to when this was written (3 tools), so it is *what already ships* rather than a new guess — 052's argument. **Superseded 2026-08-26 by 018/#124**, which added a fourth tool: the formula now gives **10**, and implementing this row as the literal 8 would silently cut turns short. Take the formula, never the number. **`thread_limit=40` is signed off, not derived** — it is 2× the edge's turn budget, a deliberate slack so the inner limit is a backstop rather than a second gate. `exit_behavior='end'` means an exhausted turn finishes with a sentence, not a stack trace. |
...
[071](https://github.com/Subaru-Goto/PanelVerdict/issues/162)). **The ceiling is unchanged at
...
- **The analyst runs the cheapest model available** — `openai/gpt-5.6-luna` since 2026-08-05,
  ...
  [070](https://github.com/Subaru-Goto/PanelVerdict/issues/161).
...
made it depend on [063](https://github.com/Subaru-Goto/PanelVerdict/issues/158), which had the load-bearing
...
- **what the app does at the ceiling.** Refusing with an error is the lazy answer; falling
  back to the demo report ([061](https://github.com/Subaru-Goto/PanelVerdict/issues/156)) means the app still works
...
- **the analyst's own limit.** It spends too. 045 argues the honest unit is *turns per thread
  per window* at the edge; [052](https://github.com/Subaru-Goto/PanelVerdict/issues/149)
