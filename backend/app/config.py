...
class Settings(BaseSettings):
    ...
    # Every model string below is the one a recorded measurement was taken
    # against, and `tests/test_config.py::_MEASURED_ON` names which document
    # each one would make stale. Changing one is fine; shipping the change
    # without rerunning what it invalidates is what 106/#226 is about — the
    # role-play guard alone is ~160 paid calls, so the rerun cannot live in CI
    # and the test is the reminder instead.
