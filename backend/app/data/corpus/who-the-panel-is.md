...
asked to choose between your two headlines.

This matters for how you read the result. The panel is not a survey of real
readers and their preferences are not evidence of what real readers do. What the
panel offers is a fast, cheap, repeatable reading of how two pieces of copy land
on a described audience — useful for comparing two options, not for estimating a
click-through rate.
...
**Grounded in:** `app/panel.py`; the report's own note that every voter is a
sampled persona, not a person.
...
**Grounded in:** `app/data/joint`, the OECD joint distribution tables;
`docs/research/persona-seed-data.md`.
...
describes a different way of reading the same headline.

**Grounded in:** `app/panel.py`, the trait level descriptions;
`docs/research/persona-seed-data.md`.
...
drawn to novelty than most people are — it does not mean they are adventurous in
some fixed sense, and it does not translate into a score on any questionnaire
anyone has taken.
...
**Grounded in:** `app/bigfive.py` — *"country does not condition the Big Five
μ"*; Donnellan & Lucas, cited in `docs/research/persona-seed-data.md`.

## What this panel cannot tell you

The honest limit, stated because it bounds every verdict on the report.

The panel has been tested on headlines that say genuinely different things, and it
separates them. It has **not** been shown to track human preference between two
headlines that say the *same* thing in different words — and that second case is
what a lot of real copy testing is.

So a decisive verdict between two clearly different propositions is the case this
product has evidence for. A decisive verdict between two rephrasings of one idea
is the case where the panel may be confidently wrong, and the report should be
read as one input rather than as an answer.

**Grounded in:** `docs/research/task-framing.md`; the *Known limitations* section
