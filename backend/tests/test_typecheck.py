...
def mypy(tmp_path: Path, name: str, source: str) -> str:
    ...
    run = subprocess.run(
        [sys.executable, "-m", "mypy", "--config-file", str(BACKEND / "pyproject.toml"), str(module)],
        capture_output=True,
        text=True,
        cwd=BACKEND,
...
def test_a_double_missing_a_protocol_method_is_reported_inside_a_test(tmp_path: Path) -> None:
