...
def _education_df(rows: list[tuple[str, str, str, float]]) -> pd.DataFrame:
...
def test_combine_blends_edu_bands_within_a_dl_band():
    ...
    def weight(edu: EducationLevel) -> float:
        cell = joint[(joint["age_band"] == "30-39") & (joint["education"] == edu.value)]
        return cell["weight"].sum()

    # tertiary: 100*0.5 + 300*0.3 = 140 ; secondary: 100*0.5 + 300*0.7 = 260
    assert weight(EducationLevel.TERTIARY) == pytest.approx(140.0)
    assert weight(EducationLevel.SECONDARY) == pytest.approx(260.0)
    assert joint["weight"].sum() == pytest.approx(400.0)
...
def test_combine_applies_floor_fraction_to_youngest_group():
    ...
    population = pd.DataFrame([("Y15T19", "M", 500.0)], columns=["age", "sex", "count"])
    education = _education_df([("Y25T34", "M", "ISCED11A_3_4", 1.0)])
    joint = combine(population, education)
    # 500 * (2/5) * 1.0 = 200; the 15-17 portion is dropped.
    cell = joint[(joint["age_band"] == "18-19") & (joint["gender"] == "male")]
    assert cell["weight"].sum() == pytest.approx(200.0)
    assert joint["weight"].sum() == pytest.approx(200.0)


def _combined_df(rows: list[tuple[str, str, EducationLevel, float]]) -> pd.DataFrame:
    return pd.DataFrame(rows, columns=["age_band", "gender", "education", "weight"])


def test_attach_income_splits_each_cell_into_five_quintiles_conserving_mass():
    combined = _combined_df([("30-39", "female", EducationLevel.TERTIARY, 100.0)])
    joint = attach_income(combined)
    assert sorted(joint["income_quintile"].tolist()) == [1, 2, 3, 4, 5]
    assert joint["weight"].sum() == pytest.approx(100.0)
...
def _skewed_joint() -> pd.DataFrame:
    ...
    rows = []
    for q in range(1, 6):
        rows.append(("30-39", "male", EducationLevel.TERTIARY, q, tertiary[q - 1]))
        rows.append(("30-39", "male", EducationLevel.BELOW_SECONDARY, q, below[q - 1]))
    return pd.DataFrame(
        rows, columns=["age_band", "gender", "education", "income_quintile", "weight"]
    )


def test_rake_income_makes_quintiles_uniform():
    raked = _rake_income(_skewed_joint())
    shares = raked.groupby("income_quintile")["weight"].sum() / raked["weight"].sum()
    assert (shares - 0.20).abs().max() < 1e-6
...
def test_parse_sdmx_csv_keys_population_by_age_and_sex():
    ...
    cell = pop[(pop["AGE"] == "Y40T44") & (pop["SEX"] == "M")]
    assert cell["value"].iloc[0] == pytest.approx(10967569.0)
...
def test_parse_sdmx_csv_keys_education_by_age_sex_attainment():
    ...
    cell = edu[
        (edu["AGE"] == "Y25T34")
        & (edu["SEX"] == "F")
        & (edu["ATTAINMENT_LEV"] == "ISCED11A_5T8")
    ]
    assert cell["value"].iloc[0] == pytest.approx(57.22052002)
...
def test_parse_sdmx_csv_skips_rows_with_no_observation():
    ...
    assert (edu["ATTAINMENT_LEV"] == "ISCED11A_5T8").all()
...
def test_education_is_incomplete():
    tertiary_only = _education_df([("Y25T34", "M", "ISCED11A_5T8", 0.6)])
    missing_below = _education_df(
        [("Y25T34", "M", "ISCED11A_3_4", 0.4), ("Y25T34", "M", "ISCED11A_5T8", 0.6)]
    )
...
def test_complete_education_from_peers():
    peers = [
        ...
        _education_df(
            [
                ("Y25T34", "M", "ISCED11A_0T2", 0.20),
                ("Y25T34", "M", "ISCED11A_3_4", 0.40),
                ("Y25T34", "M", "ISCED11A_5T8", 0.40),
            ]
        ),
    ]
...
def test_complete_education_from_peers_fills_only_the_missing_level():
    peers = [
        _education_df(
            [
                ("Y35T44", "F", "ISCED11A_0T2", 0.10),
                ("Y35T44", "F", "ISCED11A_3_4", 0.30),
                ("Y35T44", "F", "ISCED11A_5T8", 0.60),
            ]
        )
    ]
    # reports secondary + tertiary, only below-secondary missing
    target = _education_df(
...
def test_complete_education_from_peers_fails_loud_on_incomplete_peer():
    target = _education_df([("Y25T34", "M", "ISCED11A_5T8", 0.60)])  # needs below + sec
    bad_peer = _education_df(
        [("Y25T34", "M", "ISCED11A_0T2", 0.10), ("Y25T34", "M", "ISCED11A_5T8", 0.50)]
    )  # no secondary
