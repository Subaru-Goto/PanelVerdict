...
So the rule is a measurement, not a memory: walk `app/` and fail on any `async
def` that calls a known-blocking symbol outside `asyncio.to_thread`. Derived
from the tree, so a module added tomorrow is covered the day it appears.
"""
...
def _offences(tree: ast.AST, path: str) -> list[str]:
    ...
    for fn in ast.walk(tree):
        if not isinstance(fn, ast.AsyncFunctionDef):
            continue
        # What `asyncio.to_thread` was handed is exactly what is allowed.
        excused = {
            id(node.args[0])
        ...
        for node in ast.walk(fn):
            if not isinstance(node, ast.Call) or id(node.func) in excused:
                continue
...
@mark.parametrize("source", sorted(APP.rglob("*.py")), ids=lambda p: p.name)
