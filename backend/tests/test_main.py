...
@pytest.mark.anyio
async def test_an_open_that_hangs_before_libpq_is_still_bounded(
    monkeypatch, caplog
) -> None:
    """psycopg resolves the host through the shared executor before libpq's
    own timeout starts, so a queued lookup sits outside `connect_timeout`.
    The outer bound covers it: an open that never reaches libpq still answers
    503 within the deadline, and the refusal is logged by its class."""

    async def hang(cls, url: str, **kwargs: object) -> None:
        await asyncio.sleep(60)

    monkeypatch.setattr(psycopg.AsyncConnection, "connect", classmethod(hang))
    monkeypatch.setattr(main_module, "CONNECT_TIMEOUT_SECONDS", 0.05)
    dependency = get_conn()
...
def test_startup_sizes_the_shared_executor_to_the_pool(
    ...
    sizes to cpu+4 — five on a 1-vCPU container by that rule. It fronts a pool of
...
def test_a_kept_report_says_its_prose_was_ai_generated_and_by_what(
    signed_in, conn
) -> None:
    """075/#165. A stored report is containerised text under the Code of
    Practice's Sub-measure 1.1.1, so the container says what is true of it:
    generated by a model, which one, and that the prose inside carries no mark.
    Written by the server beside `timings`, so a client cannot claim otherwise;
    the row stores the same object the wire carries."""
    seed_japanese(conn, 5)
    ...
    assert provenance["generated_by"] == settings.panel.model
    assert provenance["text_marking"] == "none"
    datetime.fromisoformat(provenance["recorded_at"])  # a real timestamp, not a label
    (stored,) = [
        row[0]
        for row in conn.execute("SELECT report -> 'provenance' FROM tests").fetchall()
    ]
    assert stored == provenance


def test_an_old_stored_report_without_provenance_still_loads(signed_in, conn) -> None:
    """Reports kept before 075 shipped have no provenance key. Absent is a legal
    value — the same reading 033 gave `timings` and 070 gave `usage` — and it
    is what "kept before the field existed" honestly looks like."""
    seed_japanese(conn, 5)
    signed_in.post("/evaluate", json=_REQUEST_BODY, headers=_as("acct-a"))
    (test_id,) = [
        row[0] for row in conn.execute("SELECT test_id FROM tests").fetchall()
    ...
    conn.execute(
        "UPDATE tests SET report = report - 'provenance' WHERE test_id = %s", (test_id,)
    )
    conn.commit()
    ...
    assert stored.status_code == 200
    assert stored.json()["provenance"] is None
