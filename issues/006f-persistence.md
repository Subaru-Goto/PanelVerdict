...
- **D5 — Orchestration: dev-first validation → threaded full run.**
  ...
  - Concurrency: **bounded `ThreadPoolExecutor` (~8–16 workers)** for interest synthesis (the sync LangChain layer needs no async rewrite); **embeddings batched across personas** (`embed_documents` takes a list). **Validate correctness on the dev subset / a handful first, then enable threading for the full run.**
...
- **D6 — Testing: stub what we control, real DB for what we can't fake.**
  - **Unit** (no network/DB; stub the `InterestLLM`/`Embedder`/`Judge` Protocols): id derivation (`US-00042` format + per-slot seed **reproducibility** and **independence**), persona assembly, embedding **batch scatter/gather** (index-matching vectors back to personas), QC-report assembly from stubbed audit/eval outputs.
